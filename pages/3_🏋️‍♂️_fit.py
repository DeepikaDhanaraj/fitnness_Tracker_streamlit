...
with engine.connect() as conn:
    query_exercise = text("SELECT * FROM Exercise")
    all_exercise_results = conn.execute(query_exercise).fetchall()
...
if exercise_keyword != '':
    with engine.connect() as conn:
        query_exercise = text("SELECT * FROM Exercise WHERE Name = :name")
        exercise_result = conn.execute(query_exercise, {'name': exercise_keyword}).fetchone()
